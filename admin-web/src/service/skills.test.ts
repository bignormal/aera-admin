...
describe('skill service', () => {
  it('uses the skill catalog collection', async () => {
    ...
    expect(apiRequest).toHaveBeenNthCalledWith(
      1,
      '/skill-catalog?page=1&limit=10&sort=name&draft=true',
      {
        signal: undefined,
      },
    )
  ...
  it('sends the explicit skill distribution class', async () => {
    vi.mocked(apiRequest).mockResolvedValueOnce({ doc: { id: 4 } })
    await createSkill({
      active: true,
    ...
    expect(apiRequest).toHaveBeenCalledWith('/skill-catalog?draft=true', {
      body: expect.objectContaining({ distributionClass: 'runtime_public' }),
      method: 'POST',
    })
  })
})
