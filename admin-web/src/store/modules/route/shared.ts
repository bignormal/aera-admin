...
import { hasCapability, isAdminRole } from '@/constants/capabilities';
...
function filterAuthRouteByRoles(route: ElegantConstRoute, roles: string[]): ElegantConstRoute[] {
  ...
  const hasCapabilityPermission =
    !requiredCapability || roles.some(role => isAdminRole(role) && hasCapability(role, requiredCapability));
  ...
  return (hasPermission || isEmptyRoles) && hasCapabilityPermission ? [filterRoute] : [];
}
