...
fail() {
  printf 'Admin candidate provenance build failed: %s\n' "$*" >&2
  exit 1
}
...
test "$AERA_PROVENANCE_REPOSITORY" = "bignormal/aera-admin" ||
  fail "repository must be bignormal/aera-admin"
printf '%s\n' "$AERA_PROVENANCE_SOURCE_SHA" |
  grep -Eq '^[0-9a-f]{40}$' ||
  fail "source SHA must be 40 lowercase hexadecimal characters"
test "$AERA_PROVENANCE_WORKFLOW_PATH" = ".github/workflows/candidate.yml" ||
  fail "workflow path must be the Admin candidate workflow"
